    ...
    for packet in packets:
        ...
        if SMB2_Header in packet:
            if packet.haslayer(SMB2_Header):
                ...
                smb_path = packet[SMB2_Header].Path.decode('utf-16le', errors='ignore')
                ...
                if smb_command == 12:  # SMB_COM_SESSION_SETUP_ANDX
                    ...
                    if b'Users\\' in packet[SMB2_Header].Path:
                        packet_data.append(1)  # Mark as malicious (example)
                else:
            ...
            if packet.haslayer(SMB_Header):
                ...
                smb_path = packet[SMB_Header].Path.decode('utf-16le', errors='ignore')
                # Check for specific SMB commands or paths
                if smb_command == 12:  # SMB_COM_SESSION_SETUP_ANDX
                    packet_data.append(1)  # Mark as malicious (example)
                elif smb_command == 37:  # SMB_COM_TREE_CONNECT
                    if b'Users\\' in packet[SMB_Header].Path:
                        packet_data.append(1)  # Mark as malicious (example)
                else:
                    packet_data.append(0)  # Benign
        else:
...
df.drop(['tcp flags', 'http_request', 'http_request_payload',
         'http_response', 'http_response_payload'], axis=1, inplace=True)
...
# Drop non-numeric columns or handle them appropriately
X_train.drop(['tcp flags', 'http_request_payload', 'http_response', 'http_response_payload'], axis=1, inplace=True)
X_test.drop(['tcp flags', 'http_request_payload', 'http_response', 'http_response_payload'], axis=1, inplace=True)
